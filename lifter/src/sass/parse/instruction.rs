...
pub struct SassRegister {
    ...
    /// cNEG: conditional operand negation (SASS "-R" prefix, encoding control bit cNEG).
    /// The disassembler renders this textually as "-Rx".
    pub negated: bool,
    /// cINV: conditional operand inversion (SASS "~R" prefix, encoding control bit cINV).
    /// The operand is inverted only when the carry-in predicate (Px for IMAD.X/IADD3.X)
    /// is true.  Mutually exclusive with cNEG: an operand has at most one of cNEG/cINV.
    pub conditionally_negated: bool,
    /// cABS: conditional absolute value (SASS "|Rx|" notation, encoding control bit cABS).
    /// The disassembler renders this textually as "|Rx|".
    pub cabs: bool,
}
...
impl SassRegister {
    pub fn new(prefix: &str, number: u32) -> Self {
        Self {
            ...
            component: None,
            negated: false,
            conditionally_negated: false,
            cabs: false,
        }
    }

    /// Parse from string like "R0", "-R0", "R2.X", "|R0|", "!P0", "UR5", "P0", "RZ"
    pub fn parse(s: &str) -> Option<Self> {
        let s = s.trim();

        // cABS: conditional absolute value  →  "|Rx|"
        let (cabs, s) = if let Some(inner) = s.strip_prefix('|').and_then(|t| t.strip_suffix('|')) {
            (true, inner)
        } else {
            (false, s)
        };
        ...
        //   '-'  = cNEG (conditional negation),  '~'  = cINV (conditional invert)
        //   '!'  = cNOT (conditional NOT on predicate operands, parsed as negated)
        let (negated, conditionally_negated, s) = if let Some(stripped) = s.strip_prefix('-') {
            (true, false, stripped)
        ...
        if s == "RZ" {
            return Some(Self {
                prefix: "RZ".to_string(), number: 255,
                is_uniform: false, is_zero: true,
                component: None, negated, conditionally_negated, cabs,
            });
        }
        if s == "URZ" {
            return Some(Self {
                prefix: "URZ".to_string(), number: 63,
                is_uniform: true, is_zero: true,
                component: None, negated, conditionally_negated, cabs,
            });
        }
        if s == "PT" {
            return Some(Self {
                prefix: "PT".to_string(), number: 7,
                is_uniform: false, is_zero: true,
                component: None, negated, conditionally_negated, cabs,
            });
        }
...
pub enum SassOperand {
    ...
    Memory {
        ...
        scale: u32,
        /// True if the address is a 64-bit register pair (e.g. [R2.64])
        is_64bit_addr: bool,
        /// desc[UR] prefix — shared-memory descriptor index.
        /// e.g. `desc[UR8][R32.64+0x40]` → desc_ur = Some(UR8).
        /// Lowered upstream by stripping desc and mapping to shmem space.
        desc_ur: Option<SassRegister>,
    },
...
impl SassOperand {
    ...
    pub fn parse(s: &str) -> Option<Self> {
        ...
        // Constant bank: c[0x0][0x0] or -c[0x0][0x0]
        if s.starts_with("c[") || s.starts_with("-c[") {
            let negated = s.starts_with("-c[");
            let body = if negated { &s[1..] } else { s };
        ...
        // desc[UR][R0.64] — shared-memory descriptor prefix
        if s.starts_with("desc[") {
            let desc_end = s.find("][")?;
            let desc_inner = s.get(5..desc_end)?; // skip "desc["
            let desc_reg = SassRegister::parse(desc_inner)?;
            let mem_str = s.get(desc_end + 2..)?; // skip "["
            let mem_inner = mem_str.strip_suffix(']')?;
            let mut mem = parse_memory_operand(mem_inner)?;
            // Set desc_ur on the memory operand
            if let SassOperand::Memory { ref mut desc_ur, .. } = &mut mem {
                *desc_ur = Some(desc_reg);
            }
            return Some(mem);
        }
...
fn parse_memory_operand(inner: &str) -> Option<SassOperand> {
    ...
    // Check for .64 suffix on base or offset part
    let is_64bit = parts.iter().any(|p| p.ends_with(".64"));
    ...
    Some(SassOperand::Memory {
        ...
        index: None,
        scale: 1,
        is_64bit_addr: is_64bit,
        desc_ur: None,
    })
}

// ============================================================================
// Decoded Field — structured observation of a single bitfield decode
// ============================================================================

/// State of a decoded field: did the disassembler successfully observe it?
///
/// Follows the "失败保留事实" principle (Constitution #2):
/// - `Available` = field was present and successfully decoded
/// - `Unavailable` = field should be present but could not be extracted
/// - `NotApplicable` = this field does not apply to this instruction type
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "state")]
pub enum DecodedField<T: Serialize + Clone> {
    #[serde(rename = "available")]
    Available {
        value: T,
        /// Bit range [msb, lsb] in the 128-bit encoding where this field was read
        bit_range: [u8; 2],
    },
    #[serde(rename = "unavailable")]
    Unavailable { reason: String, bit_range: [u8; 2] },
    #[serde(rename = "not-applicable")]
    NotApplicable { evidence: String },
}

impl<T: Serialize + Clone> DecodedField<T> {
    pub fn available(value: T, msb: u8, lsb: u8) -> Self {
        DecodedField::Available {
            value,
            bit_range: [msb, lsb],
        }
    }
    pub fn unavailable(reason: &str, msb: u8, lsb: u8) -> Self {
        DecodedField::Unavailable {
            reason: reason.to_string(),
            bit_range: [msb, lsb],
        }
    }
    pub fn not_applicable(evidence: &str) -> Self {
        DecodedField::NotApplicable {
            evidence: evidence.to_string(),
        }
    }
}

// (DecodeFacts types removed — flat JSON conversion now done inline in module.rs)
...
pub fn classify_opcode(opcode: &str) -> SassOpcodeClass {
    ...
    match op.as_str() {
        ...
        | "BFE" | "BFI" | "FLO" | "POPC" | "BMSK" | "SGXT" | "BREV" => {
            SassOpcodeClass::IntegerLogical
        }

        // Integer Conversion
        "I2I" | "I2F" | "F2I" | "I2FP" => SassOpcodeClass::IntegerConversion,
        ...
        // Double Precision
        "DADD" | "DMUL" | "DFMA" | "DSETP" | "DSET" | "DSETP" => SassOpcodeClass::DoubleArithmetic,
        ...
        // Control Flow
        "BRA" | "JMP" | "JMXU" | "BSSY" | "BSYNC" | "BPT" => SassOpcodeClass::Branch,
        "BRX" | "JMX" => SassOpcodeClass::ConditionalBranch,
        "CAL" | "JCAL" | "CALL" => SassOpcodeClass::Call,
        "RET" => SassOpcodeClass::Return,
        "EXIT" => SassOpcodeClass::Exit,

        // Synchronization
        "BAR" | "DEPBAR" | "B2R" | "R2B" | "ERRBAR" | "WARPSYNC" => SassOpcodeClass::Barrier,
        "MEMBAR" => SassOpcodeClass::MemoryBarrier,
        "CCTL" | "CCTLL" | "CCTLT" => SassOpcodeClass::MemoryBarrier,
        ...
        | "USETP" | "UISETP" | "ULOP3" | "UPLOP3" | "USHF" | "VOTEU" | "UBREV" | "UBMSK"
        | "USGXT" | "R2UR" => SassOpcodeClass::UniformDataPath,
...
pub fn get_ptx_template(opcode: &str) -> PtxTemplate {
    ...
    match op.as_str() {
        ...
        "DSETP" => PtxTemplate::Direct {
            opcode: "setp".to_string(),
            default_type: Some("f64".to_string()),
        },

        // Half Precision
        "HADD2" => PtxTemplate::Direct {
            opcode: "add".to_string(),
            default_type: Some("f16x2".to_string()),
        },
        "HMUL2" => PtxTemplate::Direct {
            opcode: "mul".to_string(),
            default_type: Some("f16x2".to_string()),
        },
        "HFMA2" => PtxTemplate::Direct {
            opcode: "fma.rn".to_string(),
            default_type: Some("f16x2".to_string()),
        },
        "HSETP2" => PtxTemplate::Direct {
            opcode: "setp".to_string(),
            default_type: Some("f16x2".to_string()),
        },
        ...
        // Warp / Sync
        "SHFL" => PtxTemplate::Special {
            handler: "shfl_to_shfl_sync".to_string(),
        },
        "VOTEU" => PtxTemplate::Direct {
            opcode: "vote".to_string(),
            default_type: Some("u32".to_string()),
        },
        ...
        "CCTL" | "CCTLL" | "CCTLT" => PtxTemplate::NoEquivalent,
        "ERRBAR" => PtxTemplate::NoEquivalent,
...
mod tests {
    ...
    fn test_register_parse() {
        ...
        // cABS: |R9|
        let rabs = SassRegister::parse("|R9|").unwrap();
        assert_eq!(rabs.prefix, "R");
        assert_eq!(rabs.number, 9);
        assert!(rabs.cabs);

        // cNOT: !P0
        let pnot = SassRegister::parse("!P0").unwrap();
        assert_eq!(pnot.prefix, "P");
        assert_eq!(pnot.number, 0);
        assert!(pnot.negated);
        assert!(!pnot.conditionally_negated);

        // cNEG + cABS: |-R5|
        let nabs = SassRegister::parse("|-R5|").unwrap();
        assert!(nabs.cabs);
        assert!(nabs.negated);
        assert_eq!(nabs.prefix, "R");
        assert_eq!(nabs.number, 5);
    }
